...
function encodeBase62x(input) {
    ...
    const b62x = config.b62x;

    let inputBytes = Buffer.isBuffer(input) ? input : Buffer.from(input);
    ...
    function writeSegment(segment) {
        const [currByte, nextByte, byteAfterNext] = segment;
        ...
        if (nextByte !== undefined) {
            writeChars(c1);
        } else { // This is the last byte, shift low-bits to LSB.
            return writeChars(c1>>4);
        }

        if (byteAfterNext !== undefined) {
            writeChars(
                (nextByte<<2 & LOW_6) | byteAfterNext>>6, // MSB: 4 low of next byte; LSB: 2 high-bits of byte-after-next.
                byteAfterNext & LOW_6
            );
        } else { // Next byte is last, its 4 low bits are written as is.
            writeChars(nextByte & LOW_4);
        }
    }
